...
def create_client(
    ...
    if protocol in (Protocol.JSON2, Protocol.JSON2S):
        return Json2Client(url=url, database=database, api_key=api_key, timeout=timeout, verify=verify)

    if protocol in (Protocol.JSONRPC, Protocol.JSONRPCS):
        return JsonRpcClient(url, database, user, password, timeout, use_json2=False, verify=verify)

    return XmlRpcClient(url, database, user, password, timeout, verify=verify)
